...
# We will remove the stopwords from the texts because they do not provide much use for training our model. However, we
# will keep them for our summaries so that they sound more like natural phrases.
...
embeddings_index = {}
with open('/home/isuru/Downloads/numberbatch-en-17.06.txt/data', encoding='utf-8') as f:
    for line in f:
        values = line.split(' ')
        word = values[0]
...
# I use a threshold of 20, so that words not in CN can be added to our word_embedding_matrix, but they need to be common
# enough in the reviews so that the model can understand their meaning.
...
# To see the quality of the summaries that this model can generate, you can either create your own review, or use a review from the dataset.
# You can set the length of the summary to a fixed value, or use a random value like I have here.
